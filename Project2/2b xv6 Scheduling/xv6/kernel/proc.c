  ...
  for(;;){
    ...
    acquire(&ptable.lock);
		...
		if(flag)
		{
			...
			for(p = ptable.proc;p < &ptable.proc[NPROC];p++)
			{
				if(p->state != RUNNABLE) continue; //we only need process which is RUNNABLE
					if(tar[p->priority] == NULL || p->wait[p->priority] > tar[p->priority]->wait[p->priority])
						tar[p->priority] = p;
			}
			flag = 0;
		}
		...
		if(proc->done[proc->priority]%ticks[proc->priority] == 0 && proc->priority != 0)
		{
			flag = 1;
